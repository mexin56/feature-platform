...
## Phase 5.1 增量:自定义数据集(2026-06-13 用户确认)
- 模型 CustomDataset(全局不分项目):key="{source}.{dataset}" 唯一、name/desc、mode(snapshot|per_symbol)、collector_type(http_json|tushare_api)、config_json、target_table=ods_{source}_{dataset} 自动派生、created_by、审计。
- collectors/custom.py:按行构建 DataSet。http_json 配置 {url, method, headers, params, body?, records_path(点路径), field_map(列→JSON键,空=首条记录自动)};模板变量 {dt} {dt_nodash} {symbol} 作用于 url/params/body。tushare_api 配置 {api_name, params, fields},必须经 tushare_client.get_pro(),{symbol} 代入前经 _ts_code 归一化。
